...
@app.route('/speach_cmd', methods=['POST'])
def checkSpeach_cmd():
    textcmd = ""
    if 'speach_cmd' in request.form:
        print("cmd recieved")
        textcmd = request.form['speach_cmd']
    elif request.json and 'speach_cmd' in request.json:
        print("cmd json recideved")
        textcmd = request.json['speach_cmd']
...
def process_image(image_file):
    ...
    x = 0
    if image_file:
        savedImage = 'received_image.jpg'
        image_file.save(savedImage)
        socketio.send("congratualations you have sent images")
...
if __name__ == '__main__':
    ...
    socketio.run(app, host='192.168.137.1', port=5000,debug=True)  # Run on all interfaces so it's accessible from the network
